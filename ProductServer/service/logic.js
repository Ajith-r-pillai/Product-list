...
return db.subcategory.findOne({sid:sparentid}).then(result=>{
                  if(result){
                    ...
                    sgparentid=[...newarr]
                    const newCAT=new db.subcategory({sid,sname,simage,sparentid,sgparentid})
                         newCAT.save()
                       return{
                             statusCode:200,
                             message :"category detailes added"
                    ...
                    const newCAT=new db.subcategory({
                        sid,sname,simage,sparentid,sgparentid})
                         newCAT.save()
...
return db.subcategory.findOne({sid:parentid}).then(result=>{
                  if(result){
                    ...
                    pgparentid=[...newarr]
                    const newProduct=new db.product({pid,parentid,pgparentid,pname,pprice,pimage})
                    newProduct.save()
                       return{
                             statusCode:200,
                    ...
                    const newProduct=new db.product({
                        pid,parentid,pgparentid,pname,pprice,pimage})
                        newProduct.save()
        ...
        const getallProducts=(pid)=>{
            return db.product.find({$or:[{parentid:pid},{pgparentid:pid}]}).then(result=>{
                if (result) {
    ...
    module.exports={
        AddCategory,AddSubCategory,addProduct,getallCategory,getallSubCategory,getallProducts,getaCategory
